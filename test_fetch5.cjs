...
pool.query(`SELECT * FROM app_sessions LIMIT 1`).then(r => { 
  ...
  fetch("http://localhost:4174/api/dashboard/logs", { headers: { Authorization: `Bearer ${token}` } })
    ...
    .then(d=> {
      if (d.data && d.data.teacherLogs) {
        console.log(JSON.stringify(d.data.teacherLogs.filter(x=>x.name.includes("NGADMIN") || x.name.includes("ROSYI")), null, 2));
      } else {
