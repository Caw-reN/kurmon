...
import { Send, LayoutTemplate, Pin, Calendar, User, EyeOff, Eye, Trash2 } from'lucide-react';
...
export default function Pesan({
  dashboardMessageForm, setDashboardMessageForm, DASHBOARD_MESSAGE_TARGETS, DASHBOARD_MESSAGE_PRIORITIES, handleSaveDashboardMessage, dashboardMessages, handleToggleDashboardMessageSafe, handleRemoveDashboardMessageSafe,
  ...
  globalMessage,
  setGlobalMessage,
  handleSaveGlobalMessage,
  canEdit
}) {
  
        const [isSending, setIsSending] = useState(false);
    
    // Periksa role user dan permission tab ini
    const permLevel = getTabPermissionLevel("pesan");
    const localCanEdit = permLevel ==="edit" || (permLevel ==="otomatis" && activeUserRole !=="kepsek");
        if (!isLeadershipRole(currentUser?.role)) {
          return (
            <div className="ui-card p-6 text-center">
            <PageHeader title="Pesan & Pengumuman" icon={MessageSquare} description={tabSubtitles["pesan"] || tabSubtitles["default"]} />
        ...
        return (
            <div className="flex flex-col gap-6 w-full animate-in fade-in duration-300 relative z-10">
              <PageHeader
                title="Pusat Pesan & Pengumuman"
                description={tabSubtitles["pesan"] || tabSubtitles["default"]}
                icon={MessageSquare}
              />
              <div
                className={`grid grid-cols-1 ${localCanEdit ?"xl:grid-cols-[400px_1fr]" :"xl:grid-cols-1"} gap-6  w-full`}
              >
            {/* Form Buat Pesan */}
            {localCanEdit && (
