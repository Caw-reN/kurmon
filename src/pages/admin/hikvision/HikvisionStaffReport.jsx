...
export default function HikvisionStaffReport({ classes = [], isNested = false }) {
  ...
  const handlePrintPeriod = async () => {
    try {
      ...
      if (printPeriod === "harian") {
        periodTitle = `LAPORAN ABSENSI HARIAN KARYAWAN (${printDate} ${monthName.toUpperCase()} ${filter.year})`;
        periodSubTitle = `Tanggal: ${printDate} ${monthName} ${filter.year} | Target: Rekap Tenaga Kependidikan / Staff`;
        printDays = [printDate];
      } else if (printPeriod === "mingguan") {
        const startDay = (printWeek - 1) * 7 + 1;
        const endDay = Math.min(printWeek * 7, daysInMonth);
        periodTitle = `LAPORAN ABSENSI MINGGUAN KARYAWAN (MINGGU KE-${printWeek})`;
        ...
        periodTitle = `LAPORAN MATRIKS ABSENSI BULANAN KARYAWAN`;
        ...
        periodTitle = `REKAPAN ABSENSI KARYAWAN 1 SEMESTER (${printSemester.toUpperCase()})`;
          ...
          <table class="table-data">
            ...
            <tfoot>
              <tr style="background-color: #f8fafc; font-weight: bold;">
                <td colspan="3" style="text-align: right; font-weight: bold;">TOTAL KESELURUHAN KARYAWAN:</td>
  ...
  return (
    <div className="space-y-4 animate-fade-in">
                ...
                paginatedData.map((d, idx) => {
                  ...
                  return (
                    ...
                    >
                       ...
                       <td className="px-3 py-3 text-center font-black text-slate-800 border-r border-slate-100">{d.total_alpa || 0}</td>
                       {daysToRender.map((dayNum) => {
                          const dayData = d.days[dayNum];
                          return (
                             <td 
           ...
           <div className="bg-white rounded-[var(--ui-radius-card)] shadow-sm max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100">
             <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50/80">
               <div className="flex items-center gap-2">
                 ...
                 <div>
                   <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider">Cetak Laporan Kehadiran Karyawan</h3>
                   <p className="text-[10px] text-slate-500 font-medium">Pilih periode dan format pencetakan laporan karyawan</p>
                 </div>
               </div>
               <button
                 type="button"
             ...
             <div className="p-5 space-y-4 text-xs">
               <div>
                 ...
                 <div className="grid grid-cols-2 gap-2">
                   {[
                     { id: 'harian', label: 'Harian (Per Hari)', desc: 'Presensi karyawan 1 hari', icon: Calendar },
                     ...
                     { id: 'semester', label: '1 Semester', desc: 'Rekap total 6 bulan', icon: Briefcase },
                   ].map(item => (
                     <button
                       key={item.id}
                       type="button"
      ...
      {/* MODAL INPUT / EDIT SURAT IZIN / SAKIT KARYAWAN */}
      <Modal 
        isOpen={Boolean(selectedCell)} 
        onClose={() => setSelectedCell(null)}
        title="Input Surat Izin / Sakit Karyawan"
      >
        {selectedCell && (
            ...
            <form onSubmit={handleCellSubmit} className="space-y-4">
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-[var(--ui-radius-small)] p-2.5 flex items-start gap-2 text-amber-950 text-xs shadow-xs">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-amber-800 mt-0.5">
                      Karyawan tercatat hadir hanya saat absen pulang ({selectedCell.outTime || '--:--'}) tanpa scan masuk pagi.
                    </p>
                  </div>
                </div>
              )}
              <div>
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-1">Nama Karyawan</label>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Tulis alasan izin/sakit karyawan di sini..."
