...
import { Users, Filter, Search, Printer, FileText, X, Calendar, Award, Plus, Download, FileSpreadsheet, Share2, ArrowUpDown, AlertTriangle, UserX, UserCheck, Clock, Edit2, Eye, CheckCircle2, RefreshCw } from 'lucide-react';
...
export default function HikvisionTeacherReport({ isNested = false }) {
  ...
  const lateTeachersToday = useMemo(() => {
    if (!isCurrentMonthYear) return [];
    return data.filter(teacher => {
      const dayData = (teacher.days || {})[todayNum];
  ...
  const presentTeachersToday = useMemo(() => {
    if (!isCurrentMonthYear) return [];
    return data.filter(teacher => {
      const dayData = (teacher.days || {})[todayNum];
  ...
  const fetchData = useCallback(async (isManualRefresh = false) => {
    ...
    try {
      const res = await fetch("/api/hikvision/report/matrix", {
        ...
        body: JSON.stringify({ ...filter, type:'guru', force: Boolean(isManualRefresh) })
      });
      const json = await res.json();
      if (json.ok) {
        setData(json.data || []);
      ...
      filteredData.forEach((item, rowIndex) => {
        ...
        exportDays.forEach(i => {
          ...
          if (dayData) {
            ...
            const isMissingIn = !dayData.in && Boolean(dayData.out);
            
            if (isMissingIn) {
              fgColor = "FFFEF3C7"; // amber-100
              fontColor = "FFB45309"; // amber-800
      ...
      const body = filteredData.map(item => {
        ...
        exportDays.forEach(i => {
          ...
          if (!dayData) {
            ...
            } // Close if (dayData.taps && ...)
            
            const statusForColor = dayData.status || (dayData.isLate ? "Terlambat" : (dayData.in || dayData.out ? "Hadir" : "Alpa"));
            const isMissingIn = !dayData.in && Boolean(dayData.out);
            ...
            if (isMissingIn) {
              ...
              textColor = [180, 83, 9];
            } else if (statusForColor === 'Hadir') {
              fillColor = [220, 252, 231]; 
              textColor = [22, 101, 52];
            } else if (statusForColor === 'Terlambat') {
              fillColor = [254, 226, 226]; 
              textColor = [153, 27, 27];
            } else if (statusForColor === 'Sakit') {
              fillColor = [254, 243, 199]; 
              textColor = [146, 64, 14];
            } else if (statusForColor === 'Izin') {
              fillColor = [219, 234, 254]; 
              textColor = [30, 58, 138];
            } else if (statusForColor === 'Alpa' || statusForColor === 'Alpa (Tanpa Keterangan)') {
              fillColor = [15, 23, 42]; 
  ...
  return (
    <div className="space-y-4 animate-fade-in">
          ...
          <div className="flex items-center gap-2">
            ...
            {(['admin', 'superadmin', 'super_admin'].includes(String(user?.role || '').toLowerCase()) || String(user?.username || '').toLowerCase() === 'admin') && (
              <Button 
                ...
                className="px-3.5 py-2 bg-slate-900 hover:bg-slate-800 text-white border-slate-900 flex items-center justify-center gap-1.5 text-xs font-bold cursor-pointer shadow-xs"
      ...
      {subTab ==='matriks' && (
        <>
          {/* Daily Attendance KPI Summary Cards (Guru) */}
          {isCurrentMonthYear && data.length > 0 && (
            <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-4">
              ...
              >
                ...
                <div className="mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-slate-100 flex items-center justify-between text-[10px] sm:text-[11px]">
                  <span className="font-bold text-emerald-700 bg-emerald-50 px-1.5 sm:px-2 py-0.5 rounded-[var(--ui-radius-pill)] border border-emerald-200/60 text-[9px] sm:text-[10px] truncate">
                    {data.length > 0 ? Math.round((presentTeachersToday.length / data.length) * 100) : 0}% Hadir
                  </span>
              ...
              >
                ...
                <div className="mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-slate-100 flex items-center justify-between text-[10px] sm:text-[11px]">
                  <span className={`font-bold px-1.5 sm:px-2 py-0.5 rounded-[var(--ui-radius-pill)] border text-[9px] sm:text-[10px] truncate ${
                    lateTeachersToday.length > 0 
                      ? 'text-amber-700 bg-amber-50 border-amber-200/60' 
                      : 'text-slate-500 bg-slate-50 border-slate-200/60'
                  }`}>
                    {lateTeachersToday.length > 0 ? `${lateTeachersToday.length} Telat` : 'Nihil'}
                  </span>
              ...
              {/* Card 3: Guru Belum Hadir / Izin / Sakit */}
              <div 
              ...
              >
                ...
                <div className="mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-slate-100 flex items-center justify-between text-[10px] sm:text-[11px]">
                  <span className="font-bold text-rose-700 bg-rose-50 px-1.5 sm:px-2 py-0.5 rounded-[var(--ui-radius-pill)] border border-rose-200/60 text-[9px] sm:text-[10px] truncate">
                    {absentTeachersToday.length > 0 ? `${absentTeachersToday.length} Guru` : 'Nihil'}
                  </span>
      ...
      {/* Interactive Modal for Daily Attendance Lists (Guru) */}
      {dailyDetailModal && (
        <Modal
          isOpen={Boolean(dailyDetailModal)}
          onClose={() => { setDailyDetailModal(null); setDailySearchQuery(''); }}
          title="Monitoring Kehadiran Guru Hari Ini"
          maxWidth="max-w-2xl"
        >
          <div className="space-y-3.5">
            ...
            <div className="flex items-center gap-1.5 p-1 bg-[var(--ui-surface-muted)] rounded-[var(--ui-radius-control)] border border-[var(--ui-border-muted)] overflow-x-auto">
              ...
              >
                ...
                <span>Masuk ({presentTeachersToday.length})</span>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <span>Terlambat ({lateTeachersToday.length})</span>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <span>Belum Hadir ({absentTeachersToday.length})</span>
              </button>
            </div>
            ...
            <div className="border border-[var(--ui-border-muted)] rounded-[var(--ui-radius-card)] overflow-hidden divide-y divide-[var(--ui-border-muted)] max-h-[340px] overflow-y-auto custom-scrollbar bg-white">
              {(() => {
                ...
                const filtered = currentList.filter(t => {
                  ...
                  const code = String(t.nis || t.code || '').toLowerCase();
                  const subj = String(t.class_name || t.subject || '').toLowerCase();
                  return name.includes(q) || code.includes(q) || subj.includes(q);
                });
                ...
                if (filtered.length === 0) {
                  return (
                    <div className="p-8 text-center text-xs text-slate-400 font-bold flex flex-col items-center justify-center gap-2">
                      ...
                      <span>{dailySearchQuery ? 'Tidak ada guru yang sesuai pencarian.' : 'Tidak ada data guru untuk kategori ini.'}</span>
                    </div>
                  );
                }

                return filtered.map((t, idx) => {
                  const dayData = (t.days || {})[todayNum] || {};
                  ...
                  return (
                    <div key={t.nis || t.code || idx} className="p-2.5 px-3.5 flex items-center justify-between gap-3 text-xs hover:bg-[var(--ui-surface-muted)] transition-colors">
                      <div className="min-w-0 flex-1 flex items-center gap-2.5">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="font-bold text-slate-800 truncate text-[12px]" title={t.name}>{t.name}</div>
                          <div className="text-[10px] text-slate-400 font-semibold flex items-center gap-1.5 truncate">
                            <span>Kode: {t.nis || t.code || '-'}</span>
                            {(t.class_name || t.subject) && <span className="text-slate-600 font-bold">• {t.class_name || t.subject}</span>}
      ...
      {selectedTeacherForRapor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[var(--ui-radius-card)] shadow-sm w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100">
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  <button
                    ...
                    className={`py-2 px-3 rounded-[var(--ui-radius-small)] text-xs font-extrabold transition-all border cursor-pointer ${
                      raporPaperSize === 'A4'
                        ? 'border-[var(--ui-primary)] text-[var(--ui-primary)] bg-[var(--ui-primary)]/10 shadow-xs'
