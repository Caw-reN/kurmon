...
export default function KartuPelajar({ students: propStudents = [] }) {
  ...
  const [rejectDialog, setRejectDialog] = useState({ isOpen: false, item: null, note: '' });
  const [approveDialog, setApproveDialog] = useState({ isOpen: false, item: null, note: '' });
  ...
  const reloadDataAfterApproval = async () => {
    fetchRequests();
    if (authToken) {
      try {
        const studRes = await fetch('/api/data/load', { headers: { Authorization: `Bearer ${authToken}` } });
        const studData = await studRes.json();
        if (studData?.payload?.students) {
          setStudents(studData.payload.students);
        }
      } catch {}
    }
  };

  const handleApproveRequest = async (item, note = '') => {
    ...
    try {
      const response = await fetch('/api/student-card-requests', {
        ...
        body: JSON.stringify({ 
          action: 'approve', 
          id: item.id,
          admin_note: note || 'Disetujui oleh Admin / Tata Usaha'
        })
      });
      ...
      if (data.ok) {
        ...
        showToast(data.error || 'Gagal menyetujui pengajuan', 'error');
      }
    } catch (err) {
      console.error(err);
  ...
  const handleRejectRequest = async (item, note) => {
    setIsProcessingAction(true);
    try {
      const response = await fetch('/api/student-card-requests', {
        method: 'POST',
        ...
        body: JSON.stringify({ 
          action: 'reject', 
          id: item.id,
          admin_note: note || 'Ditolak oleh Admin / TU'
        })
      });
      const data = await response.json();
      if (data.ok) {
        showToast('Pengajuan telah ditolak.');
        setRejectDialog({ isOpen: false, item: null, note: '' });
        fetchRequests();
      } else {
        showToast(data.error || 'Gagal menolak pengajuan', 'error');
      }
    } catch (err) {
      console.error(err);
      showToast('Gagal menghubungi server', 'error');
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 w-full font-inherit">
      ...
      {/* ─── TAB 3: LOG & PERSETUJUAN PENGAJUAN DATA / CETAK ULANG ────────────────── */}
      {activeTab === 'pengajuan' && (() => {
        const filteredRequests = requests.filter(req => {
          if (requestStatusFilter !== 'all' && req.status !== requestStatusFilter) return false;
          if (requestSearch) {
            const q = requestSearch.toLowerCase();
            const matchName = String(req.nama || '').toLowerCase().includes(q);
            const matchNis = String(req.nis || '').toLowerCase().includes(q);
            const matchReason = String(req.alasan || '').toLowerCase().includes(q);
        ...
        const doneCount = requests.filter(r => r.status === 'selesai').length;
          ...
          <div className="bg-white border border-slate-200/80 rounded-[var(--ui-radius-card)] p-5 shadow-xs space-y-4 animate-in fade-in duration-200 font-inherit">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div>
                <h3 className="font-black text-slate-800 text-sm flex items-center gap-2">
                  ...
                  Daftar Permohonan &amp; Approval Data Siswa
                </h3>
                <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                  Tinjau dan ACC pembaruan pas foto resmi, TTL, atau cetak ulang kartu pelajar dari siswa secara langsung.
                </p>
              </div>
              ...
              <button
                ...
                className="p-2 rounded-[var(--ui-radius-control)] bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 text-xs font-bold flex items-center gap-1.5 cursor-pointer transition-all"
              >
            ...
            <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-1 md:pb-0">
                {[
                  { id: 'all', label: 'Semua', count: requests.length, color: 'bg-slate-100 text-slate-700' },
                  { id: 'pending', label: 'Menunggu ACC', count: pendingCount, color: 'bg-amber-100 text-amber-800' },
                  { id: 'disetujui', label: 'Disetujui', count: approvedCount, color: 'bg-emerald-100 text-emerald-800' },
                  { id: 'selesai', label: 'Selesai', count: doneCount, color: 'bg-sky-100 text-sky-800' },
                  { id: 'ditolak', label: 'Ditolak', count: rejectedCount, color: 'bg-rose-100 text-rose-800' },
                ].map(tab => (
                  <button
                    key={tab.id}
                    type="button"
                    onClick={() => setRequestStatusFilter(tab.id)}
                    className={`px-3 py-1.5 rounded-[var(--ui-radius-control)] text-xs font-black flex items-center gap-1.5 cursor-pointer transition-all shrink-0 ${
                      requestStatusFilter === tab.id
                        ? 'bg-[var(--ui-primary-btn,var(--ui-primary))] text-white shadow-xs'
                        : 'bg-slate-50 text-slate-600 hover:bg-slate-100 border border-slate-200/80'
                    }`}
                  >
                    <span>{tab.label}</span>
                    <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${
                      requestStatusFilter === tab.id ? 'bg-white/25 text-white' : tab.color
                    }`}>
                      {tab.count}
                    </span>
                  </button>
              ...
              <div className="relative min-w-[220px]">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-[var(--ui-radius-control)] focus:outline-none focus:border-emerald-500 font-medium"
                />
              </div>
            </div>

            {/* Request Table */}
            <div className="border border-slate-200/80 rounded-[var(--ui-radius-control)] overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead className="bg-slate-100 border-b border-slate-200/80 text-[10px] font-black uppercase text-slate-500 tracking-wider">
                  <tr>
                    <th className="p-3.5">Nama &amp; NIS Siswa</th>
                    <th className="p-3.5">Jenis Pengajuan</th>
                    <th className="p-3.5">Rincian Data yang Diajukan</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredRequests.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="p-10 text-center text-slate-400 bg-slate-50/50">
                        <Clock size={32} className="mx-auto mb-2 opacity-30" />
                        <p className="font-bold text-xs">Tidak ada permohonan yang sesuai filter.</p>
                      </td>
                    </tr>
                  ) : (
                    filteredRequests.map(reqItem => {
                      const proposed = reqItem.proposed_data || {};
                      const studentRecord = students.find(s => String(s.nis).trim() === String(reqItem.nis).trim()) || {};
                      const currentPhoto = studentRecord.photo || studentRecord.foto || null;
                      const currentTtl = studentRecord.ttl || '-';

                      return (
                        <tr key={reqItem.id} className="hover:bg-slate-50 transition-colors">
                          {/* Nama & NIS */}
                          <td className="p-3.5">
                            <div className="font-black text-slate-800">{reqItem.nama}</div>
                            <div className="text-[11px] font-mono text-slate-500 font-bold flex items-center gap-1.5 mt-0.5">
                              <span>NIS: {reqItem.nis}</span>
                              <span>&bull;</span>
                              <span className="text-emerald-700 bg-emerald-50 px-1.5 py-0.2 rounded text-[10px] border border-emerald-200">
                                {reqItem.kelas}
                              </span>
                            </div>
                          </td>
                          ...
                          {/* Jenis Pengajuan */}
                          <td className="p-3.5">
                            <span className="inline-flex px-2.5 py-1 rounded-[var(--ui-radius-small)] text-[10px] font-black uppercase tracking-wider bg-slate-100 text-slate-700 border border-slate-200">
                              {reqItem.request_type === 'update_foto_ttl' ? 'Foto & TTL' :
                               reqItem.request_type === 'update_foto' ? 'Pas Foto Saja' :
                               reqItem.request_type === 'update_ttl' ? 'TTL Saja' : 'Cetak Fisik'}
                            </span>
                          </td>
                          ...
                          <td className="p-3.5 max-w-xs">
                            <div className="space-y-1.5">
                              <p className="text-slate-700 font-medium leading-relaxed">
                                <span className="font-bold text-slate-900">Alasan:</span> {reqItem.alasan}
                              </p>

                              {/* Preview Usulan Foto */}
                              {proposed.photo && (
                                <div className="flex items-center gap-2 bg-slate-100/80 p-1.5 rounded-[var(--ui-radius-small)] border border-slate-200/80">
                                  <div className="relative group cursor-pointer" onClick={() => setImageZoomUrl(proposed.photo)}>
                                    <img
                                      src={proposed.photo}
                                      alt="Foto Usulan"
                                      className="w-9 h-11 object-cover rounded-[var(--ui-radius-small)] border border-slate-300 shadow-2xs group-hover:opacity-80 transition-all"
                                    />
                                    <span className="absolute inset-0 bg-black/30 rounded-[var(--ui-radius-small)] opacity-0 group-hover:opacity-100 flex items-center justify-center text-white text-[9px] font-black transition-opacity">
                                      Zoom
                                    </span>
                                  </div>
                                  <div className="text-[10px] min-w-0">
                                    <span className="font-bold text-slate-800 block text-emerald-700">Usulan Foto Baru</span>
                                    <span className="text-slate-400 block truncate">Klik foto untuk perbesar</span>
                                  </div>
                                </div>
                              )}
                              ...
                              {/* Usulan TTL */}
                              {proposed.ttl && (
                                <div className="text-[11px] p-1.5 rounded-[var(--ui-radius-small)] bg-amber-50/80 border border-amber-200 text-amber-900">
                                  <div><span className="text-slate-400 font-semibold">TTL Usulan:</span> <strong className="text-slate-900">{proposed.ttl}</strong></div>
                                  <div className="text-[10px] text-slate-500 mt-0.5"><span className="text-slate-400">Saat ini:</span> {currentTtl}</div>
                                </div>
                              )}

                              {/* Catatan Admin jika ada */}
                              {reqItem.admin_note && (
                                <div className="text-[10px] p-1.5 rounded bg-slate-100 text-slate-600 font-medium">
                                  <span className="font-bold text-slate-700">Respon TU:</span> {reqItem.admin_note}
                                </div>
                              )}
                            </div>
                          </td>
                          ...
                          <td className="p-3.5 text-center">
                            <span className={`inline-flex px-2.5 py-0.5 rounded-[var(--ui-radius-small)] text-[10px] font-black uppercase tracking-wider ${
                              ...
                              reqItem.status === 'disetujui' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
                              reqItem.status === 'selesai' ? 'bg-sky-50 text-sky-700 border border-sky-200' : 'bg-rose-50 text-rose-700 border border-rose-200'
                            }`}>
                              {reqItem.status === 'pending' ? 'Menunggu ACC' :
                            ...
                            <div className="flex items-center justify-end gap-1.5">
                                ...
                                <>
                                  <button
                                    ...
                                    onClick={() => setApproveDialog({ 
                                      isOpen: true, 
                                      item: reqItem, 
                                      note: 'Pengajuan telah disetujui & data kartu langsung diperbarui.' 
                                    })}
                                    className="px-3 py-1.5 rounded-[var(--ui-radius-small)] bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[11px] shadow-xs cursor-pointer flex items-center gap-1 transition-all active:scale-98"
                                    title="ACC dan Terapkan Data Siswa"
                                  >
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => setRejectDialog({ isOpen: true, item: reqItem, note: '' })}
                                    className="px-2.5 py-1.5 rounded-[var(--ui-radius-small)] bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 text-[11px] font-black cursor-pointer transition-all"
                                  >
                                    Tolak
                                  </button>
                                </>
                                ...
                                <button
                                  ...
                                  onClick={async () => {
                                    ...
                                    setTimeout(() => {
                                      ...
                                      showToast('Kartu dicetak & status selesai!');
                                      fetchRequests();
                                    }, 400);
                                  }}
                              ...
                              {(reqItem.status === 'selesai' || reqItem.status === 'ditolak') && (
                                <button
                                  type="button"
                                  onClick={async () => {
                                    if (window.confirm('Hapus log riwayat pengajuan ini?')) {
                                      const response = await fetch('/api/student-card-requests', {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${authToken}` },
                                      ...
                                      if (response.ok) {
                                        showToast('Log pengajuan berhasil dihapus!');
                                        fetchRequests();
                                      }
                                    }
                                  }}
      ...
      {/* Modal ACC / Setujui Pengajuan Siswa */}
      {approveDialog.isOpen && approveDialog.item && (
        <Modal
          isOpen={true}
          onClose={() => setApproveDialog({ isOpen: false, item: null, note: '' })}
          title="Konfirmasi Persetujuan (ACC) Data Siswa"
        >
          <div className="space-y-4 font-inherit">
            <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-[var(--ui-radius-control)] text-xs text-emerald-900 flex items-start gap-2.5">
              <CheckCircle2 size={18} className="text-emerald-600 shrink-0 mt-0.5" />
              <div>
                <p className="font-black">Otomatis Memperbarui Data Master Siswa</p>
                <p className="text-[11px] text-emerald-700 mt-0.5 font-medium leading-relaxed">
                  Menyetujui pengajuan ini akan langsung menimpa data foto dan/atau TTL pada database master siswa. Siswa dapat langsung mengunduh kartu baru seketika.
                </p>
              </div>
            </div>

            <div className="bg-slate-50 p-3.5 rounded-[var(--ui-radius-control)] border border-slate-200/80 text-xs space-y-2">
              <div className="flex justify-between">
                <span className="text-slate-400 font-semibold">Nama Siswa:</span>
                <span className="font-bold text-slate-900">{approveDialog.item.nama}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400 font-semibold">NIS:</span>
                <span className="font-bold font-mono text-slate-800">{approveDialog.item.nis}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400 font-semibold">Kelas:</span>
                <span className="font-bold text-slate-800">{approveDialog.item.kelas}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400 font-semibold">Alasan Siswa:</span>
                <span className="font-medium text-slate-700 text-right max-w-xs">{approveDialog.item.alasan}</span>
              </div>
            </div>

            {/* Preview of proposed changes */}
            {approveDialog.item.proposed_data?.photo && (
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-[var(--ui-radius-control)] flex items-center gap-3">
                <img
                  src={approveDialog.item.proposed_data.photo}
                  alt="Usulan Foto"
                  className="w-12 h-15 object-cover rounded-[var(--ui-radius-small)] border border-slate-300 shadow-xs shrink-0"
                />
                <div className="text-xs">
                  <p className="font-black text-slate-800">Pas Foto Baru Terlampir</p>
                  <p className="text-[11px] text-slate-500 font-medium">Foto ini akan disimpan sebagai foto resmi siswa di sistem.</p>
                </div>
              </div>
            )}

            {approveDialog.item.proposed_data?.ttl && (
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-[var(--ui-radius-control)] text-xs">
                <p className="text-slate-400 font-semibold text-[10px] uppercase">Tempat, Tanggal Lahir Baru:</p>
                <p className="font-black text-slate-900 text-sm mt-0.5">{approveDialog.item.proposed_data.ttl}</p>
              </div>
            )}
            ...
            <div>
              ...
              <input
                ...
                placeholder="Contoh: Pengajuan telah disetujui TU..."
                className="w-full px-3 py-2 bg-white border border-slate-300 rounded-[var(--ui-radius-control)] text-xs font-semibold focus:outline-none focus:border-emerald-500"
              />
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 flex justify-end gap-2">
              <Button 
                variant="outline" 
                type="button" 
                onClick={() => setApproveDialog({ isOpen: false, item: null, note: '' })}
              ...
              <Button
                type="button"
                disabled={isProcessingAction}
                onClick={() => handleApproveRequest(approveDialog.item, approveDialog.note)}
                className="bg-emerald-600 hover:bg-emerald-700 text-white font-black px-4 rounded-[var(--ui-radius-control)] flex items-center gap-1.5"
              >
                <CheckCircle2 size={15} />
                <span>{isProcessingAction ? 'Memproses...' : 'ACC & Terapkan Data'}</span>
              </Button>
            </div>
          </div>
        </Modal>
      ...
      {/* Modal Tolak Pengajuan Siswa */}
      {rejectDialog.isOpen && rejectDialog.item && (
        <Modal
          isOpen={true}
          onClose={() => setRejectDialog({ isOpen: false, item: null, note: '' })}
          title="Tolak Pengajuan Siswa"
        >
          <div className="space-y-4 font-inherit">
            <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-[var(--ui-radius-control)] text-xs text-rose-900">
              <p className="font-black">Konfirmasi Penolakan Pengajuan</p>
              <p className="text-[11px] text-rose-700 mt-0.5 font-medium leading-relaxed">
                Pengajuan dari <strong>{rejectDialog.item.nama}</strong> (NIS: {rejectDialog.item.nis}) akan ditolak. Berikan alasan agar siswa dapat memperbaikinya.
              </p>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Contoh: Pas foto belum menggunakan seragam OSIS lengkap / Foto buram / Format TTL tidak sesuai akta..."
                className="w-full px-3 py-2 bg-white border border-slate-300 rounded-[var(--ui-radius-control)] text-xs font-semibold focus:outline-none focus:border-rose-500"
              />
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 flex justify-end gap-2">
              <Button 
                variant="outline" 
                type="button" 
                onClick={() => setRejectDialog({ isOpen: false, item: null, note: '' })}
                disabled={isProcessingAction}
              >
                Batal
              </Button>
              <Button
                type="button"
                disabled={isProcessingAction}
                onClick={() => handleRejectRequest(rejectDialog.item, rejectDialog.note || 'Pengajuan ditolak oleh TU/Admin.')}
                className="bg-rose-600 hover:bg-rose-700 text-white font-black px-4 rounded-[var(--ui-radius-control)]"
              >
                {isProcessingAction ? 'Memproses...' : 'Tolak Pengajuan'}
              </Button>
            </div>
          </div>
        </Modal>
      ...
      {imageZoomUrl && (
        <div 
          className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs cursor-pointer"
          onClick={() => setImageZoomUrl(null)}
        >
          <div className="relative max-w-sm w-full bg-white p-3 rounded-[var(--ui-radius-card)] shadow-2xl space-y-3 cursor-default" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
              <span className="font-black text-xs text-slate-800 flex items-center gap-1.5">
                <ImageIcon size={14} className="text-emerald-600" /> Pratinjau Foto Usulan Siswa
              </span>
              <button
                type="button"
                onClick={() => setImageZoomUrl(null)}
            ...
            <div className="rounded-[var(--ui-radius-control)] overflow-hidden bg-slate-100 border border-slate-200 flex items-center justify-center min-h-[320px] max-h-[460px]">
              <img src={imageZoomUrl} alt="Foto Zoom" className="w-full h-full object-contain max-h-[460px]" />
            </div>
